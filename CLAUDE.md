...
| `tests/` | `test_scripts.py` (Python, stdlib `unittest`, no network; also run by the workflow before every build), `security.mjs`, `axe.mjs`, `lighthouse.mjs`, `screenshots.mjs`, `mock-data.mjs`, `server.mjs` | Yes; add a test with each fix |
...
```bash
npm test                  # Python checks, security, axe (72 combinations), then Lighthouse; exits non-zero if anything fails
npm run test:py           # the Python scripts (resolve(), calendar escaping and folding, CazéTV matching, research.py input checks); no Node needed: python3 -m unittest discover -s tests
