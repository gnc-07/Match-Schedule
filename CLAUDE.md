...
| `build_schedule.py` | Builds `fixtures.json` and `soccer.ics` from the league feed, `overrides.json` and `friendlies.json`, plus F1 sessions from Jolpica-F1 (cross-checked with OpenF1 through `resolve()`; F1 stays out of `soccer.ics` and the 20-match safety check) | Yes, carefully |
...
npm test                  # script layout, Python checks, security, axe (72 combinations), then Lighthouse; exits non-zero if anything fails
...
npm run test:axe          # axe-core WCAG 2.2 AA: light/dark, en/pt, 390/1280px; menus closed, Settings, Calendar, Tables, match details, F1 race weekend, high contrast
