...
- On a wide monitor the panel opens beside the list instead of over it, so you can keep scrolling and click other matches while it stays open.
...
- On laptops and wider screens the filters sit in a column on the left. **Hide filters** folds them away (a bar above the list then says what is shown, with a **Show filters** button); the choice is remembered on the device. Wide monitors also get a column on the right with the live scores and the key to the labels. With Large or Extra large text, the page uses fewer columns automatically.
