...
| `tests/` | Checks that are never published: `test_scripts.py` (the Python scripts, including the rule for when a time counts as verified), `security.mjs` (hostile data must never run in the page), and the accessibility and Lighthouse checks. |
